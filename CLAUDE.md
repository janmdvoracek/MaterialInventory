...
The suite lost its one `TransactionTestCase` (`StockLockConcurrencyTests`) along with `select_for_update()`. If you ever reintroduce a concurrent write path, it needs a `TransactionTestCase` again — a plain `TestCase` wraps the whole test in one transaction and hides the interleaving.
