...
- `inventory` — `StockMovement`, an **append-only ledger**. Current stock is always derived by summing signed quantities per material/location, never stored as a mutable counter. `movement_type` is one of `RECEIPT`, `SHIPMENT`, `TRANSFORM_CONSUME`, `TRANSFORM_PRODUCE`, `ADJUSTMENT` (the last restricted to Manager/Admin). `inventory/services.py::get_available_quantity(material, location, lock=False)` is the shared stock-check helper, used with `lock=True` inside `transaction.atomic()` to close check-then-write races on shipments and transform-consume. `inventory/views.py` also has a filterable, paginated movement history view plus a streaming CSV export. **`created_at` is the time the movement *happened*, not `auto_now_add`** — it defaults to `timezone.now` and the Receipt/Shipment forms let a worker override it via a checkbox (`MovementTimingMixin` in `inventory/forms.py`), because depot work gets written up hours late. Everything sorts, filters and exports by it. The companion `recorded_at` *is* `auto_now_add` and keeps the audit trail honest about when a back-dated row was actually entered. Two consequences: code that writes a movement must decide whether to pass `created_at`, and the locked stock check deliberately ignores it — back-dating never changes what's available now.
...
- **Time is 24-hour everywhere the app renders it** — templates use `H:i`, the CSV export `%H:%M:%S`, Czech `TIME_FORMAT` is `G:i`. The `<input type="datetime-local">` on Příjem/Výdej is the one exception and is *not* ours to control: browsers format that widget from the device's own language, so an English-configured phone shows AM/PM regardless of `lang="cs"`. Swapping it for a text field taking `20.08.2026 07:30` would force 24h but lose the mobile picker; that was weighed and rejected, same call as `localize=False` on quantities. Don't re-open it without new information.
...
1. **App labels** — `verbose_name` on each `AppConfig` (`Katalog`, `Sklad`, `Zakázky`, `Uživatelé`), which is what the admin index groups by.
...
CI (`.github/workflows/ci.yml`) runs `ruff check` and `ruff format --check` (must pass, not just run), a `docker build` sanity check, and a test job as separate jobs on every push/PR. The test job is three steps, not one: `manage.py check`, then `manage.py makemigrations --check --dry-run`, then `manage.py test`. That middle step means **a model change without its migration file fails CI even if every test passes** — run `makemigrations` and commit the result alongside any `models.py` edit. Both triggers carry a `paths-ignore` list covering `**.md` plus dotfiles and dot-directories at any depth, so a commit touching only those runs **no jobs at all** — don't wait for a green check on such a push, and don't take one as evidence the code still builds. The exempt set works out to `*.md`, `.gitignore`, `.dockerignore`, `.env.example`, and everything under `.github/`; `Dockerfile` and `docker-compose.yml` are not dotfiles and still trigger CI. Two exempt files *do* affect CI — `.dockerignore` (what lands in the image) and the workflow itself — so a change to either is unverified until the next code push. Dependabot (`.github/dependabot.yml`) opens weekly update PRs for pip, Docker base images, and the Actions themselves.
