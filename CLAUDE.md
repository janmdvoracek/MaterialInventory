...
1. **App labels** — `verbose_name` on each `AppConfig` (`Katalog`, `Zakázky`, `Uživatelé`; `inventory` carries `Zpracování` but registers nothing, so it is not shown), which is what the admin index groups by.
