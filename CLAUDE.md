...
- `materials` — `Material` catalog (SKU, unit of measure, category), `Location` (depot zones/bins), and `Machine` (name, `is_active`, `total_hours` — a simple running counter, not derived from usage records — and two optional reference rates, `hourly_rate` in Kč/hod and `rate_per_ton` in Kč za tunu zpracovaného materiálu — neither is used in any calculation). `materials/management/commands/seed_data.py` bulk-loads materials/locations/machines/users from CSV (see Commands below).
