...
**A job does not count until it is approved.** `WorkOrder` carries `status` (`PENDING`/`APPROVED`/`RETURNED`), `reviewed_at`, `reviewed_by` and `review_note`. `transform_create` writes a worker's submission as `PENDING`; a manager's or admin's own submission is written `APPROVED` with themselves as reviewer, because there is nobody above them to sign it off. **`time_worked`, `machine_usage_history` and `machine_dashboard` all filter to `status=APPROVED`** — a pending job is recorded but invisible in every report, including to its own author, so the submit message tells the worker it is waiting. Migration `0008` back-filled every pre-existing job to `APPROVED` (reviewed_at = created_at, no reviewer): they were counted before the rule existed and must not vanish. The review pages are `job_dashboard` / `job_detail` / `job_edit` / `job_approve` / `job_return` / `job_delete`, all under `/jobs/` and all gated with `@role_required(MANAGER, ADMIN)`. Returning requires a note — `transform_create` shows the requesting user their own `RETURNED` jobs with it, and that notice is all a worker gets: **only a manager edits a job**. Editing deliberately does *not* approve. `job_edit` reuses the submission's own helpers (`_collect_rows`, `_balance_error`, `_write_job_rows`), so a correction is mass-balanced exactly like the original, and it writes the rows as the **author** (their hours in „Moje hodiny", `collaborator_queryset` built from the author so it excludes them and not the manager, `created_by=author` on the movements).
