...
- `materials` — `Material` catalog (SKU, unit of measure, category), `Location` (depot zones/bins), and `Machine` (name, `is_active`, `total_hours` — a simple running counter, not derived from usage records). `materials/management/commands/seed_data.py` bulk-loads materials/locations/users from CSV (see Commands below).
...
`machines.csv` only takes a `name` column — a machine's `total_hours` is never set by seeding, only accumulated by transformations that log usage against it.
