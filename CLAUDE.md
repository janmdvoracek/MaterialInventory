...
`machines.csv` takes `name,hourly_rate,rate_per_ton` — a machine's `total_hours` is never set by seeding, only accumulated by transformations that log usage against it. `materials.csv` takes `sku,name,unit_of_measure,category`.
